...
class ELIM(BasicTask):
    ...
    def set_model(self):
        ...
        if opt.model == 'alexnet':
            ...
            encoder = MlpMixer(config, img_size=224, num_classes=2, patch_size=16, latent_dim=opt.ermfc_input_dim, zero_head=True)
        ...
        m_opt = torch.optim.Adam(ermfc.parameters(),       lr = 0.25*opt.e_lr, betas = (0.5, 0.99))
        ...
        self.c_lr_scheduler = lr_scheduler.MultiStepLR(self.c_opt, milestones=[5e2,10e2,20e2,30e2,50e2], gamma=0.8)
...
#                        print(fg256("cyan", len(ll_dictionary[ll_sorted[rnd_sample[abc]]])))
#                        if len(ll_dictionary[ll_sorted[rnd_sample[abc]]]) < opt.topk:
#                            print(fg256("red", '|ID_list| < topk'))
#                            continue
